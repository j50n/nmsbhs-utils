...
class TripAdvisor {
    public route: () => IRoute = lazily(() => this.rc.findRoute(this.start.coords, this.destination.coords));

    constructor(public rc: RouteCalculator, public start: IEndPoint, public destination: IEndPoint) {}
    ...
    public explain(): void {
        ...
        const systems: List<IEndPoint> = (this.route().hops.flatMap(hop => [hop.blackhole, hop.exit]) as List<IEndPoint>)
            .unshift(this.start)
